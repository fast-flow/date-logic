...
/******/ 	__webpack_require__.p = "/date-logic/0.1.8";
/******/
...
var _dateLogic = __webpack_require__("./lib/index.js");

var _dateLogic2 = _interopRequireDefault(_dateLogic);
...
var ChangeDays = function (_Component) {
    _inherits(ChangeDays, _Component);

    function ChangeDays(props) {
        _classCallCheck(this, ChangeDays);

        var _this = _possibleConstructorReturn(this, (ChangeDays.__proto__ || Object.getPrototypeOf(ChangeDays)).call(this, props));
    ...
    _createClass(ChangeDays, [{
        key: 'componentWillMount',
        value: function componentWillMount() {
            var self = this;
            self.dateL = new _dateLogic2.default({
                date: '2018-05-18',
                startWeekDay: '3',
                onChange: function onChange(data) {
                    ...
                    self.setState({
                        date: data.date,
                        monthData: (0, _extend2.default)(true, [], data.render)
                    });
                }
            });
        }
        ...
        value: function render() {
            var self = this;
            var columnTextMap = { '1': 'Mon', '2': 'Tue', '3': 'Wed', '4': 'Thu', '5': 'Fri', '6': 'Sat', '7': 'Sun' };

            return __webpack_require__("./node_modules/react/index.js").createElement(
                'div',
                { className: 'changeDaysDemo demo' },
                ...
                __webpack_require__("./node_modules/react/index.js").createElement(
                    ...
                    __webpack_require__("./node_modules/react/index.js").createElement(
                        'span',
                        { className: 'changeDays-tool-text' },
                        self.dateL.toFormat({
                            in: 'YYYY-MM-DD',
                            date: self.dateL.date,
                            output: 'YYYY年MM月DD日'
...
ChangeDays = __webpack_require__("./node_modules/react-hot-loader/index.js").hot(module)(ChangeDays);
module.exports = ChangeDays;
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__("./node_modules/webpack/buildin/module.js")(module)))

/***/ }),

/***/ "./doc/changeDays.demo.js":
/***/ (function(module, exports, __webpack_require__) {
...
var ChangeDays = __webpack_require__("./doc/ChangeDays.js");
ReactDOM.render(__webpack_require__("./node_modules/react/index.js").createElement(ChangeDays, null), document.getElementById('change-days-demo'));
...
var _fecha = __webpack_require__("./node_modules/fecha/fecha.js");
...
var DateLogic = function DateLogic(props) {
    var _this = this;

    _classCallCheck(this, DateLogic);

    this.toFormat = function (settings) {
        var self = _this;
        settings = settings || {};
        ...
        var dateObj = _fecha2.default.parse(settings.date, settings.in);
        return _fecha2.default.format(dateObj, settings.output);
    };

    this.dateToStr = function (dateObj, formatStr) {
        var self = _this;
        return _fecha2.default.format(dateObj, formatStr);
    };

    this.strToDate = function (dateStr, formatStr) {
        var self = _this;
        // console.log('strToDate : ', defaultDateStr ,dateStr , self.format )
        return _fecha2.default.parse(dateStr, formatStr) || _fecha2.default.parse(self.date, self.format);
    };

    this.updateWeekDayCol = function (firstWeekDay) {
        var self = _this;
        // 判断有效数字 非法则不更新
        ...
        self.weekDayColumn = (0, _extend2.default)(true, [], weekDayColumn);
        return weekDayColumn;
    };

    this.getThisMonthData = function (dateStr) {
        var self = _this;

        dateStr = dateStr || self.date;
        var dateObj = self.strToDate(dateStr, self.format);
        var dateArray = self.dateToStr(dateObj, 'YYYY-MM-DD').split('-');
        ...
        var firstDayWeekDay = new Date(firstDay).getDay() || 7; // 获取1号是星期几
        var dayLength = new Date(year, month, '0').getDate(); // 获取这个月的天数

        var thisMonthData = []; // 这个月的渲染数据
        // let todayString = this.today.toString().replace(/\d{2}\:\d{2}\:\d{2}.+$/,'')
        for (var i = 1; i <= dayLength; i++) {
            var tempDate = self.toFormat({
                in: 'YYYY-MM-D',
            ...
            var tempData = {
                ...
                date: tempDate
                // 判断是否添加isToday属性
            };var isToday = self.today == tempDate;
            if (isToday) {
                tempData.today = true;
            }
            // 更新星期
    ...
    this.getMonthData = function () {
        var self = _this;
        // 获取当月日历渲染数据
        var thisMonthData = self.getThisMonthData().map(function (item) {
            item.thisMonth = true;
            return item;
        });

        // 根据 当月1号星期几 计算 上月需要显示天数
        var lastMonthLength = self.weekDayColumn.indexOf(thisMonthData[0].weekDay);
        // 判断是否需要获取上个月数据
        var lastMonthData = [];
        if (lastMonthLength > 0) {
            var dateObj = self.strToDate(self.date, self.format);
            var lastMonthDateObj = dateObj;
            lastMonthDateObj.setMonth(dateObj.getMonth() - 1);
            var lastMonthDateStr = self.dateToStr(lastMonthDateObj, self.format);
            // 上月一整个月数据
            ...
            // 截取上个月需要显示的数据
            lastMonthData = lastMonthData.slice(lastMonthData.length - lastMonthLength, lastMonthData.length);
        }

        // 根据 当月最后一天星期几 计算 下月需要显示天数
        var nextMonthLength = 6 - self.weekDayColumn.indexOf(thisMonthData[thisMonthData.length - 1].weekDay);
        // 判断是否需要获取下个月数据
        var nextMonthData = [];
        if (nextMonthLength > 0) {
            var _dateObj = self.strToDate(self.date, self.format);
            var nextMonthDateObj = _dateObj;
            nextMonthDateObj.setMonth(_dateObj.getMonth() + 1);
            var nextMonthDateStr = self.dateToStr(nextMonthDateObj, self.format);
            // 下月一整个月数据
        ...
        // 结合日历渲染数据
        var monthData = lastMonthData.concat(thisMonthData).concat(nextMonthData);
        return monthData;
    };

    this.getData = function () {
        return _this.getMonthData();
    };

    this.change = function (settings) {
        var self = _this;
        settings = settings || {};
    ...
    this.lastMonth = function () {
        ...
        var result = self.getMonthData();
        var date = self.toFormat({ date: self.date });
        self.onChange({
            today: self.toFormat({ date: self.today }),
            date: date,
            weekDayColumn: self.weekDayColumn,
    ...
    this.onChange = props.onChange || function () {
        console.warn('/node_module/date-logic/index.js 未配置onChange');
    };
    // init run
    this.onChange({
        today: this.today,
...
module.exports = __webpack_require__("./doc/changeDays.demo.js");
